...
<div align="center">
  ...
  <a href="https://huggingface.co/stepfun-ai/Step-Audio-R1.1"><img src="https://img.shields.io/static/v1?label=Step-Audio-R1.1&message=HuggingFace&color=yellow"></a> &ensp;
...
- open/use embedded hosted voice chat immediately
- speak with mic/upload audio and hear model-native decoded voice replies
...
If capacity is still being assigned, use:
```bash
python runpod/deploy_pod.py --no-wait
```
...
### ⬇️ Download Model
First, you need to download the Step-Audio-R1.1 model weights.
...
```
2.  **Start the service**:
    Assuming the model is downloaded in the `Step-Audio-R1.1` folder in the current directory.
...
git clone https://github.com/Legalphoenix/stepfun.git r1-scripts
cd r1-scripts
